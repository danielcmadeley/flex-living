...
import { getRateLimitStatus, getClientIdentifier } from "../lib/rate-limit";
...
async function testRedisConnectivity() {
  ...
  try {
    ...
    log(`❌ Redis connection error: ${error instanceof Error ? error.message : 'Unknown error'}`, 'red');
    return false;
  }
}
...
async function makeTestRequest(endpoint: string, method: string = 'GET', body?: any): Promise<{
  success: boolean;
  status: number;
...
async function testEndpointRateLimit(endpoint: string, expectedType: string, maxRequests: number = 10) {
  ...
  for (let i = 1; i <= maxRequests; i++) {
    ...
    if (result.headers['x-ratelimit-limit']) {
      log(`  Rate Limit: ${result.headers['x-ratelimit-remaining']}/${result.headers['x-ratelimit-limit']} remaining`, 'cyan');
    }
    ...
    if (result.status === 429) {
      ...
      log(`  ⚠️ Request failed (${result.status}): ${result.error || 'Unknown error'}`, 'yellow');
    }
...
async function testMultipleEndpoints() {
  ...
  for (const test of endpointsToTest) {
    const result = await testEndpointRateLimit(test.endpoint, test.type, test.maxRequests);
...
async function testAdminEndpoint() {
  ...
  if (getResult.success && getResult.data) {
    ...
    log(`  Redis Status: ${getResult.data.redis?.status || 'unknown'}`, 'cyan');
    log(`  Client ID: ${getResult.data.client?.identifier || 'unknown'}`, 'cyan');

    if (getResult.data.configurations) {
      log(`  Configurations loaded: ${getResult.data.configurations.length}`, 'cyan');
    }
  } else {
    log(`❌ Admin endpoint failed: ${getResult.error || 'Unknown error'}`, 'red');
  }
  ...
  if (postResult.success && postResult.data) {
    ...
    log(`  Client ID: ${postResult.data.test?.clientId || 'unknown'}`, 'cyan');
    log(`  Successful requests: ${postResult.data.test?.summary?.successful || 0}`, 'cyan');
    log(`  Blocked requests: ${postResult.data.test?.summary?.blocked || 0}`, 'cyan');
  } else {
    log(`❌ Rate limit testing failed: ${postResult.error || 'Unknown error'}`, 'red');
  }
}

async function generateReport(testResults: any[]) {
  ...
  testResults.forEach(result => {
    ...
    log(`  Rate limit hit: ${result.rateLimitHit ? '✅ Yes' : '❌ No'}`, result.rateLimitHit ? 'green' : 'red');
  });
...
async function main() {
  ...
  try {
    ...
    if (!redisConnected) {
      ...
      log('Please check your UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN environment variables', 'yellow');
      process.exit(1);
    }
    ...
    log(`\n❌ Test suite failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'red');
    console.error(error);
    process.exit(1);
  }
}
