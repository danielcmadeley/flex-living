...
export function UrlStateProvider({
  ...
  const filtersToUrlParams = React.useCallback((filters: FilterState): URLSearchParams => {
  ...
  const createShareableUrl = React.useCallback((customFilters?: Partial<FilterState>) => {
  ...
  }, [enabled, isInitialized, parseUrlToFilters, setFilters]);
...
export function useUrlFilters() {
  ...
  const getFilterFromUrl = React.useCallback((key: string) => {
    return searchParams.get(key);
  }, [searchParams]);

  const hasFilter = React.useCallback((key: string) => {
...
export function useDeepLinking() {
  ...
  const navigateToReviews = React.useCallback((filters: Partial<FilterState>) => {
  ...
  const navigateToProperty = React.useCallback((listingName: string, additionalFilters?: Partial<FilterState>) => {
    const filters = { listingName, ...additionalFilters };
    navigateToReviews(filters);
  }, [navigateToReviews]);

  const navigateToLowRated = React.useCallback((maxRating: number = 3) => {
    navigateToReviews({ maxRating, status: 'published' });
  }, [navigateToReviews]);

  const navigateToRecent = React.useCallback((days: number = 7) => {
    const from = new Date();
